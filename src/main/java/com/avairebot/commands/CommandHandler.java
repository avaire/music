...
public class CommandHandler {
    ...
    public static CommandContainer getCommand(Message message, @Nonnull String command) {
        ...
        for (Map.Entry<List<String>, CommandContainer> entry : COMMANDS.entrySet()) {
            String commandPrefix = entry.getValue().getCommand().generateCommandPrefix();
        ...
        for (String trigger : command.getTriggers()) {
            for (Map.Entry<List<String>, CommandContainer> entry : COMMANDS.entrySet()) {
                for (String subTrigger : entry.getKey()) {
                    if (Objects.equals(CategoryHandler.getPrefix() + trigger, entry.getValue().getDefaultPrefix() + subTrigger)) {
                        throw new InvalidCommandPrefixException(CategoryHandler.getPrefix() + trigger, command.getName(), entry.getValue().getCommand().getName());
                    }
                }
            }
        }
